...
public class DotTreeGUI extends DrawingGUI {
	...
	public DotTreeGUI() { super("dottree", width, height); }
	...
	public void handleMousePress(int x, int y) {
		...
		if (mode == 'a') {
			...
			// for every time the mouse is clicked, if the tree is null, start a new tree
			// and then start adding to this tree.
			mouseX = x;
			mouseY = y;
			// assign instance variables
			if (tree == null) tree = new PointQuadtree<Dot>(new Dot(mouseX,mouseY), 0,0,800,600); // start with A
			// new tree
			tree.insert(new Dot(mouseX,mouseY));
			...
			// creates new array list of found points and adds points within query
			// circle within the point.
			found = new ArrayList<>();
			for (Dot dot : tree.allPoints()) {
				// loop though dots in circle and add points that are in circle
				if (Geometry.pointInCircle(dot.getX(),dot.getY(),mouseX,mouseY,mouseRadius)) {
					found.add(dot);
				}
			}
	...
	private void test2() {
		...
		found = null;
		tree = new PointQuadtree<Dot>(new Dot(300,400),0,0, 800,600); // Start with A
		tree.insert(new Dot(200,22)); // B
		tree.insert(new Dot(100,200)); // C
		tree.insert(new Dot(600,20)); // D
		tree.insert(new Dot(150,450)); // E
		tree.insert(new Dot(250,550)); // F
		tree.insert(new Dot(450,202)); // G

		int bad = 0;
		bad += testFind(200,300,400,7,7,6); // rect for all A [D[G]] [B[C]] [E[F]]; circle for all; can't reach D
		bad += testFind(300,300,10,6,4,0); // rect for all but E, Expected in circle A, B, G, D, hit none
		bad += testFind(10,300,10,5,3,0); // rect for B, C, E, F, A, Expected in circle A, B, C. hit none
		bad += testFind(20,300,40,5,3,0); // rect for B, C, E, F, A; circle A, B, C; hit none.
